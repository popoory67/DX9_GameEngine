...
D3D11Ptr D3D11::_instance = nullptr;
...
void D3D11::Init( HWND hWnd, bool screenMode, float screenWidth, float screenHeight )
{
	HRESULT							result;
	...
	if (FAILED( result ))
	{
		assert( Util::ErrorMessage( "failed CreateDXGIFactory" ) );
	}
	...
	if (FAILED( result ))
	{
		assert( Util::ErrorMessage( "failed EnumAdapters" ) );
	}
	...
	if (FAILED( result ))
	{
		assert( Util::ErrorMessage( "failed EnumOutputs" ) );
	}
	...
	if (FAILED( result ))
	{
		assert( Util::ErrorMessage( "failed GetDisplayModeList" ) );
	}
	...
	if (!displayModeList)
	{
		assert( Util::ErrorMessage( "DXGI_MODE_DESC is null" ) );
	}
	...
	if (FAILED( result ))
	{
		assert( Util::ErrorMessage( "failed GetDesc" ) );
	}
	...
	if (error != 0)
	{
		assert( Util::ErrorMessage( "failed to get graphic card name" ) );
	}
	...
	if (FAILED( result ))
	{
		assert( Util::ErrorMessage( "failed D3D11CreateDeviceAndSwapChain" ) );
	}
	...
	if (FAILED( result ))
	{
		assert( Util::ErrorMessage( "failed GetBuffer" ) );
	}
	...
	if (FAILED( result ))
	{
		assert( Util::ErrorMessage( "failed CreateRenderTargetView" ) );
	}
	...
	if (FAILED( result ))
	{
		assert( Util::ErrorMessage( "failed CreateTexture2D" ) );
	}
	...
	if (FAILED( result ))
	{
		assert( Util::ErrorMessage( "failed CreateDepthStencilState" ) );
	}
	...
	if (FAILED( result ))
	{
		assert( Util::ErrorMessage( "failed CreateDepthStencilView" ) );
	}
	...
	if (FAILED( result ))
	{
		assert( Util::ErrorMessage( "failed CreateRasterizerState" ) );
	}
	...
	_orthoMatrix = XMMatrixOrthographicLH( screenWidth, screenHeight, SCREEN_NEAR, SCREEN_DEPTH );
}
